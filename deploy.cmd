...
REM IF NOT EXIST "%DEPLOYMENT_TARGET%\env\azure.env.%PYTHON_RUNTIME%.txt" (
REM   IF EXIST "%DEPLOYMENT_TARGET%\env" (
REM     echo Deleting incompatible virtual environment.
REM     rmdir /q /s "%DEPLOYMENT_TARGET%\env"
REM     IF !ERRORLEVEL! NEQ 0 goto error
REM   )

REM   echo Creating %PYTHON_RUNTIME% virtual environment.
REM   %PYTHON_EXE% -m %PYTHON_ENV_MODULE% env
REM   IF !ERRORLEVEL! NEQ 0 goto error

REM   copy /y NUL "%DEPLOYMENT_TARGET%\env\azure.env.%PYTHON_RUNTIME%.txt" >NUL
REM ) ELSE (
REM   echo Found compatible virtual environment.
REM )
